...
grep -E '^__version__ = ".+"$' "$ROOT/panos_util/__init__.py" >/dev/null
sed -i.bak -E "s/^__version__ = \".+\"$/__version__ = \"$NEW_VERSION\"/" "$ROOT/panos_util/__init__.py" && rm "$ROOT/panos_util/__init__.py.bak"
